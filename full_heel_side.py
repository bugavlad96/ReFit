...
while True:
    ...
    if results.pose_landmarks:
        ...
        if (int(angle_deg_AC) >= 165 and int(angle_deg_EG) >= 165) and (counter_left == counter_right == 0):
            ...
            cv2.putText(img, "a little more...", (150, 150), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 255), 2)
            ...
            if (int(angle_deg_AC) > 65 and int(angle_deg_EG) > 65) and (int(angle_deg_AC) <= 165 and int(angle_deg_EG) <= 165):
                ...
                cv2.circle(img, points[26], 15, (0, 0, 255), cv2.FILLED)
                cv2.circle(img, points[28], 15, (0, 0, 255), cv2.FILLED)
                cv2.putText(img, "try to bring SLOWLY your knees as close as possible to the ground...", (150, 150), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 255), 2)
